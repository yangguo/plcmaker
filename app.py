...
from checkrule import searchByItem, searchByName, searchByNamesupa
from gptfuc import add_ruleindex, build_ruleindex, delete_db
from maker import df2section, exclude_row, groupbysection, savedf2csv, txt2df
from utils import get_folder_list, get_section_list  # combine_df_columns,
...
def main():
    ...
    if match == "格式转换":
        ...
        section_input = st.sidebar.text_input(
            "输入制度段落模版",
            value="第\w章 第\w节 第\w??\w?\w条 ^\d\.\s ^\d\.\d\s ^\d\.\d\.\d\s",
        )
        # input exclude text list
        exclude_text = st.sidebar.text_input("输入排除文本", value="^\d\d?$")

        # checkbox for save to db
        save_checkbox = st.sidebar.checkbox("是否保存", value=False)

        # checkbox for embedding
        st.sidebar.checkbox("是否编码", value=False)
        ...
        if st.sidebar.button("格式转换"):
            ...
            if plc_text == "" or plc_name == "":
                ...
                #         # save embedding
                #         saveembedding(embeddings,plc_name,save_path)
        ...
        build_model = st.sidebar.button("新建模型")
        if build_model:
            with st.spinner("正在生成模型..."):
                build_ruleindex(fullresultdf, industry_choice)
                st.success("模型生成完成")

        add_model = st.sidebar.button("添加模型")
        if add_model:
